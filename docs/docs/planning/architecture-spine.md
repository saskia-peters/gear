...
| 12 | `tool_types` | Equipment categories (chainsaw, generator…) + default schedule, mode, required qualification | Tool | The equipment catalogue; defines a **default schedule** (FK → `schedules`) + pass/fail or checklist mode (FR-8). Label + `attributes JSONB`. The required qualification is a SINGLE FK column `required_qualification_id` → `qualifications` (FR-8/FR-11 — the pre-epic join-table name `tool_type_qualifications` is superseded). |
| 13 | `tool_type_checklist_items` | The ordered checklist questions/tasks a tool type defines (position, label) | Tool | Definition of checklist-based inspection (FR-8, FR-12, FR-23). First `position`-ordered child table — ordering is code-level, full-replacement on update. |
| 14 | `tools` | Individual physical items, each belonging to a tool type | Tool | The actual equipment on site. Optional **`schedule_id`** (per-tool schedule choice, FK → `schedules`; null = use tool type default) (FR-9) + `attributes JSONB` (FR-10). |
| 15 | `inspections` | Every inspection run (header): tool, inspector, date, result | Tool | The inspection log (FR-18). Never deleted; anonymised on user deletion (FR-24). |
| 16 | `inspection_items` | Per-checkline results for checklist inspections | Tool | Checklist details, if the tool type is checklist mode (FR-12). |
| 17 | `reinstatements` | Out-of-Service clearing events (actor, reason, date) | Tool | Who cleared a tool Out-of-Service and why (AD-9). Sole exit from OOS (AD-4). |
| 18 | `password_reset_tokens` | Single-use, expiring credential-recovery tokens (hashed value, user, expiry, agent/approver) | User | Powers FR-26 forgot-password reset and FR-27 dual-admin recovery. One active token per user; new request invalidates older ones (AD-13). |
| 19 | `smtp_settings` | Admin-configured SMTP email-delivery parameters (host, port, security, sender, username, encrypted password) | Admin | System-wide config, edited once from the Admin UI (FR-28, AD-14). Password encrypted at rest; write-only/masked. Consumed by the User module's email adapter via Admin's settings port. |
| 20 | `backup_destinations` | Admin-configured backup targets (type S3/FTP/etc., endpoint, bucket/path, credentials, schedule) | Admin | Where automated backups are delivered (NFR-R3, AD-15). Edited once from the Admin UI (FR-29). Credentials encrypted at rest; masked in UI. |
| 21 | `schedules` | The **named schedule catalog** (e.g. "1 year", "1 quarter", "1 month", "2 weeks", "3 days"): name + V1 interval (unit/magnitude) + reserved weekday-set & time-of-day fields for future composite timing | Admin | Cross-cutting config reusable by future modules (FR-30, AD-16). Tool Types (`default_schedule_id`) and Tools (`schedule_id`) reference rows by FK; admins create/edit via `schedules.manage`. Weekday/time fields nullable & unused in V1 (cron-like, AD-16). |

> **Attribution:** tables 1–11 and **18** belong to the **User** hexagon, 12–17 to the **Tool** hexagon, and **19–21** (operational settings: `smtp_settings`, `backup_destinations`, `schedules`) to the **Admin** hexagon. The Admin module owns these three small configuration tables (AD-14/AD-15/AD-16) and no others — it otherwise configures through the Tool module's config port and the User module's lifecycle/credential-recovery ports (AD-8, AD-10, AD-13).
...
- **Centralized, data-driven permission gating (future enhancement)** — currently the **surface→permission mapping is hardcoded in code**: the composition-root gate table in `cmd/server/main.go` (~14 mounts), the per-module core re-check constants (AD-6, ~20 Go consts across `internal/{tools,admin,dsgvo,user}/core`), and the SPA's nav/route/in-page gating (`web/src/auth/permissions.ts` `ADMIN_NAV_ENTRIES` + `adminNavCodes`, `web/src/App.tsx` `RequireAdminEntry` codes, and raw code literals in `AdminDsgvoPage.tsx`, `AdminBenutzerPage.tsx`, `AdminRollenPage.tsx`, `AdminQualifikationenPage.tsx`, `AdminPage.tsx`). The same code string (e.g. `tools.manage`) is repeated across migrations, route mounts, core re-checks, and the SPA. The permission **sets** are already data-driven and resolved live per request (`ListPermissionsByUser`), and the permission **label** catalog is already server-served (`available_permissions`); only the **surface→code mapping** is not.
  **Planned enhancement (no code changes at this time):** a data-driven *Surface Gate Registry* — a DB table mapping every gated surface → its required permission code(s) + policy, consumed by the backend mounts, the core AD-6 re-checks, and (as a served endpoint) the SPA nav/routes/in-page gates — so changing which permission gates a page becomes a data edit (SQL or a future admin "Zugriffsrechte" surface, audited per NFR-O2), not a code change. The server remains the sole enforcement point (AD-6); the SPA gate is visibility-only and fails closed. See `deferred-work.md` (2026-09-19, "permission and gating architecture") for the full captured analysis.
