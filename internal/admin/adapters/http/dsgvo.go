...
// orchestrator's report generation. Implemented by *dsgvocore.Service.
...
// DsgvoRoutes returns the DSGVO admin router (Story 3.3, FR-24/AD-6): the GET
// report handler at /reports/{userId}. The composition root mounts this router
// at /api/v1/admin/dsgvo behind the DSGVO any-of gate; the router carries no
// gateway itself. 404/405 answer with the uniform JSON envelope so no sub-path
// can emit a plain-text body. Read-only — the deletion surface is Story 3.4.
...
// 3.3, AD-6). A non-holder → the generic no-hint 403 (no personal data, AD-6);
// an unknown target user → the German 404; the default branch → the clean 500
// (a canceled request is not answered).
func (h *DsgvoHandler) mapDsgvoError(w http.ResponseWriter, r *http.Request, err error, user *usercore.User) {
	...
	case errors.Is(err, dsgvocore.ErrForbidden):
		h.log().Warn("dsgvo report access forbidden", "email", user.Email)
	...
	default:
		...
		h.log().Error("dsgvo report request failed unexpectedly", "error", err)
