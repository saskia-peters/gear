...
- **B1/B2 boundary semantics pinned (2026-09-20, Epic 4 retro items B1/B2):** documented + tested that update-time FK re-validation is ACTIVE-only, so a LIVE tool whose type was archived answers 400 `MsgToolInvalidType`, and a LIVE tool whose override schedule was archived answers 400 `MsgToolInvalidSchedule` (the write must fail, not persist a dangling FK; the type/schedule must be reactivated first). Covered by `TestUpdateToolLiveToolArchivedType` + `TestUpdateToolLiveToolArchivedSchedule` at the core. The create-side matrix rows (CREATE_BAD_TYPE/CREATE_BAD_OVERRIDE) already pinned the same contract; this closes the UPDATE-side gap the retro flagged.
