...
- ~~`TestComposedAdminRouteGroup`~~ **RESOLVED (2026-09-19, Story 7.4):** each DB-backed test package now runs in its own PostgreSQL schema (`internal/platform/dbtest`), so parallel `go test ./...` never shares `users`/`sessions` rows across packages. The test passes deterministically in default parallel mode.

## Deferred from: code review (2026-09-09) of spec-4-3b-inventory-number-tool-edit.md

- ~~The SAME cross-package parallel-DB contention~~ **RESOLVED (2026-09-19, Story 7.4):** per-package test schemas give each suite its own tables, rows AND sequences (the collision-retry test's exact `tools_inventory_number_seq` assertions are now deterministic). `TestPostgresToolInventoryArchivedBackstop`, `TestPostgresCreateToolInventoryCollisionRetry`, `TestComposedToolCreateAutoAssignsInventory` all pass in default parallel mode. `ci.yml` dropped the `-p 1` workaround.
...
- ~~`TestPostgresCreateToolsBatchPreservesExplicitOrder`~~ **RESOLVED (2026-09-19, Story 7.1 CI):** the test over-asserted that the multi-row batch INSERT returns rows in INPUT order, which PostgreSQL does not guarantee for `RETURNING` over an `unnest` INSERT. Production is safe — the repo attributes batch failures by NAME (`createdNames`), never by RETURNING order, so per-row error lines stay correct. The test was relaxed to assert the created SET by name (`TestPostgresCreateToolsBatchReturnsAllInputs`), which is the actual contract; the pipeline gate now runs green.

## Deferred from: user request (2026-09-19) — permission and gating architecture

- summary: Centralize the permission/gating architecture into a data-driven "Surface Gate Registry" so changing which permission gates a page is a data edit, not a code change.
  evidence: Captured analysis (no code changes at this time): the surface→permission mapping is hardcoded in code today — (a) composition-root gate table `cmd/server/main.go` (~14 mounts), (b) per-module core re-check consts (AD-6, ~20 Go consts in internal/{tools,admin,dsgvo,user}/core), (c) SPA nav/route/in-page gating (web/src/auth/permissions.ts `ADMIN_NAV_ENTRIES` + `adminNavCodes`, web/src/App.tsx `RequireAdminEntry` codes, and raw code literals in AdminDsgvoPage.tsx, AdminBenutzerPage.tsx, AdminRollenPage.tsx, AdminQualifikationenPage.tsx, AdminPage.tsx). The same code string (e.g. tools.manage) repeats across migrations, route mounts, core re-checks, and the SPA. Permission SETS are already data-driven + resolved live per request (ListPermissionsByUser, no cache); the permission LABEL catalog is already server-served (available_permissions). Only the surface→code mapping is not data-driven.
  recommended: A DB table (e.g. surface_gates) mapping each gated surface → required code(s) + policy (any_of, extensible to all_of), consumed by the backend mounts, the core AD-6 re-checks, and served to the SPA (nav entries + route guards + in-page gates) so gates can change without code changes — optionally with an admin "Zugriffsrechte" surface later. Server remains the sole enforcement point (AD-6); SPA gate is visibility-only and fails closed. Full plan captured in docs/docs/planning/architecture-spine.md "Deferred" section.

## Deferred from: Story 7.1 CI pipeline (2026-09-19)
