...
- **OOS-rule correction (2026-09-14, surfaced in the Story 5.4 review, user-approved):** the derivation previously marked a tool OOS only when the LATEST INSPECTION was a fail — so a passing inspection implicitly cleared OOS, contradicting AD-4 ("latest FAILED inspection not since reinstated") and FR-15 (reinstatement is the SOLE exit from OOS). The rule is corrected to: OOS iff the latest FAILED inspection's `submitted_at` is at-or-after the latest reinstatement (a pass inspection never clears OOS; only a reinstatement does). `deriveToolStatus` now takes `latestFailAt` and the status read returns it (`GetLatestFailedInspection`); the SPA consequence (Story 5.4) is driven by this server-derived status, which is now genuinely authoritative. Known-bad state avoided: a volunteer pass submit silently un-blocking an OOS tool. KEEP: the OOS tie boundary, the append-only records, the never-stored-as-a-flag invariant.
