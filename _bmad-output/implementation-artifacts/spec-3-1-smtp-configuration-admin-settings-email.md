...
- **As-built addition (2026-09-14):** the Sendetest-E-Mail action no longer sends implicitly to the acting admin's address. The SPA opens a new lightweight accessible `PromptDialog` (`web/src/components/PromptDialog.tsx`, single-input modal: focus-in/return-focus, Escape, inline error, ≥48px targets) asking for the receiver, validates it client-side (empty / missing `@` → German inline error, no request), and `POST /api/v1/admin/settings/smtp/test` carries `{ "to": ... }`. The backend `TestSmtpSettings` now takes the client-supplied recipient and rejects missing/malformed/oversized (or control-char-bearing) addresses with the 400-class sentinel (German `MsgSmtpTestRecipientInvalid`) — never sent, never audited as an attempt. The generic delivery-failure result, audit (`admin.settings.email.test`) and write-only password semantics are unchanged. Implemented outside a formal story; recorded here for provenance.
